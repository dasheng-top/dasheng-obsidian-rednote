...
export class RedSettingTab extends PluginSettingTab {
    ...
    private renderBasicSettings(containerEl: HTMLElement): void {
        ...
        // 排版模式设置
        const layoutModeSetting = new Setting(typographyContent)
            .setName('排版模式')
            .setDesc('“按标题分割”需要 Markdown 标题约束；“自由排版”自动按内容高度分页')
            .addDropdown(dropdown => dropdown
                .addOption('heading', '按标题分割')
                .addOption('free', '智能分割')
                .setValue(this.plugin.settingsManager.getSettings().layoutMode || 'free')
                .onChange(async (value: 'heading' | 'free') => {
                    await this.plugin.settingsManager.updateSettings({
                        layoutMode: value,
                        templateId: value === 'free' ? 'free' : 'default'
                    });
                    new Notice('排版模式已更新，请重启 Obsidian 或重新加载以完全生效');
                    this.display();
                })
            );

        // 内容分割标题级别设置（仅按标题分割模式显示）
        const currentLayoutMode = this.plugin.settingsManager.getSettings().layoutMode || 'free';
        if (currentLayoutMode === 'heading') {
