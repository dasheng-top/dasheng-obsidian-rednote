...
export class ThemeManager {
    ...
    private currentLineHeight: number = 1.6;
    private app: App;
    ...
    public applyTheme(element: HTMLElement, theme?: Theme): void {
        ...
        if (imagePreview) {
            ...
            imagePreview.style.lineHeight = String(this.currentLineHeight);
        }
        ...
        element.querySelectorAll('p').forEach(el => {
            if (!el.parentElement?.closest('p') && !el.parentElement?.closest('blockquote')) {
                el.setAttribute('style', `${styles.paragraph}; font-family: ${this.currentFont}; font-size: ${this.currentFontSize}px; line-height: ${this.currentLineHeight}; margin: 9px 0;`);
            }
        });
        ...
        element.querySelectorAll('li').forEach(el => {
            el.setAttribute('style', `${styles.list.item}; font-family: ${this.currentFont}; font-size: ${this.currentFontSize}px; line-height: ${this.currentLineHeight};`);
        });
        element.querySelectorAll('.task-list-item').forEach(el => {
            el.setAttribute('style', `${styles.list.taskList}; font-family: ${this.currentFont}; font-size: ${this.currentFontSize}px; line-height: ${this.currentLineHeight};`);
        });

        // 应用引用样式
        element.querySelectorAll('blockquote').forEach(el => {
            el.setAttribute('style', `${styles.quote}; font-family: ${this.currentFont}; font-size: ${this.currentFontSize}px; line-height: ${this.currentLineHeight};`);
        });

        // 应用代码样式
        element.querySelectorAll('pre').forEach(el => {
            el.setAttribute('style', `${styles.code.block}; font-size: ${this.currentFontSize}px; line-height: ${this.currentLineHeight};`);
        });
        ...
        element.querySelectorAll('th').forEach(el => {
            el.setAttribute('style', `${styles.table.header}; font-family: ${this.currentFont}; font-size: ${this.currentFontSize}px; line-height: ${this.currentLineHeight};`);
        });
        element.querySelectorAll('td').forEach(el => {
            el.setAttribute('style', `${styles.table.cell}; font-family: ${this.currentFont}; font-size: ${this.currentFontSize}px; line-height: ${this.currentLineHeight};`);
        });
    ...
    public setLineHeight(lh: number) {
