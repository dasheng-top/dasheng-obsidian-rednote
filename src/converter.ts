...
export class RedConverter {
    ...
    static formatContent(element: HTMLElement): void {
        ...
        // 自由排版模式：将所有内容放入一个 section，由 FreeTemplate + AutoPaginator 分页
        if (settings?.layoutMode === 'free') {
            this.formatContentAuto(element);
            return;
        }
    ...
    /**
     * 自动模式：不拆分标题，将全部内容放入一个 tall section
     */
    private static formatContentAuto(element: HTMLElement): void {
        // 收集所有内容（深拷贝）
        const allContent = Array.from(element.childNodes).map(node => node.cloneNode(true));

        if (allContent.length === 0 || !element.textContent?.trim()) {
            element.empty();
            element.createEl('div', {
                cls: 'red-empty-message',
                text: '请先输入一些内容，系统会自动分页'
            });
            element.dispatchEvent(new CustomEvent('content-validation-change', {
                detail: { isValid: false },
                bubbles: true
